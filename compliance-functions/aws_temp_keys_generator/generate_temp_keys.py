...
IDPENTRYURL = \
    'https://sts.global.tesco.org/adfs/ls/IdpInitiatedSignOn.aspx?loginToRp=urn:amazon:webservices'


def process_creds(user, passwd):
    """Process the user credentials and return a DOC response Tesco adfs site"""
...
def generate_tmp_keys(response, role, account):
    ...
    if len(awsroles) > 1:
        ...
        principal_arn = "arn:aws:iam::" + account + ":saml-provider/TescoADFS"
        validroles = []
...
def lambda_handler(event, context):
    ...
    validroles = ['tesco-app-tester', 'tesco-app-productowner', \
    'tesco-app-developer', 'tesco-app-admin']
    # We perform a test to see if the provided role is valid
    if event['Role'] in validroles:
        uname = event['Username'] + "@global.tesco.org"
        ...
        raise Exception("The role '" + event['Role'] + "' is not a valid role")
