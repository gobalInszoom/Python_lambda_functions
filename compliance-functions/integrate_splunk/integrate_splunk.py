...
ROLE_NAME = os.environ.get("ROLE_NAME", "tesco-readsplunk-role")
BUCKET = os.environ.get("S3_BUCKET", "tesco-roles-arns")
POLICYNAME = 'tesco-splunk-read-all'

# Create the Tesco Read All Policy in the target account
def create_policy(access, secret, session):
    ...
    for policy in listofpolicies['Policies']:
        ...
        # UPDATE TO CONST defined & CHANGE TO tesco-read-all ROLE
        if policy['PolicyName'] == POLICYNAME:
            logging.warning("The tesco-splunk-policy and role already exists")
            sys.exit(0)

    # Use the policy outlined in the json file provided
    policydoc = json.load(open('tesco-splunk-read.json'))
...
# Create the Tesco Read Role in the target account
def create_role(access, secret, session, policy):
    ...
    for role in listofroles['Roles']:
        ...
        if role['RoleName'] == ROLE_NAME:
            logging.warning("The tesco-splunk-read role already exists")
            sys.exit(0)
